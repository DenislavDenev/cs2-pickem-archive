> **RETROACTIVE RELIABILITY TEST** -- This outcome grades a point-in-time reconstruction: the prediction above it was generated retroactively using only data available on 2025-06-02, with the same model that powers the current Major's live predictions (bo1_edge_v7). It exists to test that model's reliability on resolved results. It is not a pre-tournament prediction.
...
| -------------------- | ----- | --------------------- |
| Advance Brier        | 0.248 | 0.250                 |
| Advance Log-loss     | 0.691 | 0.693                 |
...
- **HEROIC** (slot 0-3_pick_1): model gave 25.6% confidence -- did not hit.
- **FlyQuest** (slot 0-3_pick_2): model gave 20.6% confidence -- did not hit.
- **TYLOO** (slot 3-0_pick_1): model gave 24.2% confidence -- did not hit.
- **Fluxo** (slot 3-0_pick_2): model gave 21.0% confidence -- did not hit.
- **B8** (slot advance_1): model gave 65.4% confidence -- did not hit.
- **NRG** (slot advance_3): model gave 55.4% confidence -- did not hit.
- **Complexity** (slot advance_5): model gave 49.8% confidence -- did not hit.
- **Chinggis_Warriors** (slot advance_6): model gave 49.4% confidence -- did not hit.
